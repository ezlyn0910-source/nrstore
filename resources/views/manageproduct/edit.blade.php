...
@section('styles')
    @vite(['resources/sass/app.scss', 'resources/css/manage_product/edit.css', 'resources/js/app.js'])
@endsection
...
<div class="product-form-container">
    ...
    <form id="productForm" method="POST" action="{{ route('admin.manageproduct.update', $product) }}" enctype="multipart/form-data" class="product-form">
        @csrf
        @method('PUT')
        ...
        <div class="form-section">
            ...
            <div class="form-checkboxes">
                ...
                <label class="checkbox-container">
                    <input type="checkbox" name="has_variations" value="1" id="has_variations" {{ old('has_variations', $product->has_variations) ? 'checked' : '' }}>
                    <span class="checkmark"></span>
                ...
                <div class="main-image-upload">
                    ...
                    @if($product->image)
                        <div class="current-image-info">
                            <small>Current image: {{ basename($product->image) }}</small>
                        </div>
                    @endif
                    @error('main_image')
                        <div class="form-error">{{ $message }}</div>
                ...
                <div class="gallery-images-upload">
                    ...
                    <div class="existing-gallery-images">
                        ...
                        <div class="image-grid">
                            ...
                            <div class="image-upload-box gallery-image-box existing-image">
                                <div class="image-preview">
                                ...
                                <div class="image-actions">
                                    <button type="button" class="btn btn-remove remove-existing-image" data-image-id="{{ $image->id }}" title="Remove image">
        ...
        <!-- Product Variations -->
        <div class="form-section variations-section" id="variations-section" style="{{ old('has_variations', $product->has_variations) ? '' : 'display: none;' }}">
            <div class="section-header">
            ...
            <div class="existing-variations">
                <h4>Existing Variations</h4>
                @foreach($product->variations as $variation)
                <div class="variation-item existing-variation" data-variation-id="{{ $variation->id }}">
                    <div class="variation-header">
                        <h4>Variation: {{ $variation->sku }}</h4>
                        <button type="button" class="btn btn-remove remove-existing-variation" data-variation-id="{{ $variation->id }}">
                            <i class="fas fa-times"></i>
                        </button>
                    </div>
                    <div class="variation-content">
                        <div class="form-grid">
                            <div class="form-group">
                                <label>SKU *</label>
                                <input type="text" name="existing_variations[{{ $variation->id }}][sku]" value="{{ $variation->sku }}" required>
                            </div>
                            <div class="form-group">
                                <label>Price (RM)</label>
                                <input type="number" name="existing_variations[{{ $variation->id }}][price]" step="0.01" min="0" value="{{ $variation->price }}">
                            </div>
                            <div class="form-group">
                                <label>Stock *</label>
                                <input type="number" name="existing_variations[{{ $variation->id }}][stock]" min="0" value="{{ $variation->stock }}" required>
                            </div>
                            <div class="form-group">
                                <label>Model</label>
                                <input type="text" name="existing_variations[{{ $variation->id }}][model]" value="{{ $variation->model }}">
                            </div>
                            <div class="form-group">
                                <label>Processor</label>
                                <input type="text" name="existing_variations[{{ $variation->id }}][processor]" value="{{ $variation->processor }}">
                            </div>
                            <div class="form-group">
                                <label>RAM</label>
                                <input type="text" name="existing_variations[{{ $variation->id }}][ram]" value="{{ $variation->ram }}">
                            </div>
                            <div class="form-group">
                                <label>Storage</label>
                        ...
                        <div class="variation-image-upload">
                            ...
                            <div class="image-upload-box variation-image-box">
                                ...
                                <input type="file" name="existing_variations[{{ $variation->id }}][image_file]" accept="image/*" class="image-input">
                            </div>
                            @if($variation->image)
                                <div class="current-image-info">
                                    <small>Current image: {{ basename($variation->image) }}</small>
                                </div>
                            @endif
                        </div>
                        <div class="form-checkboxes">
                            <label class="checkbox-container">
                                <input type="checkbox" name="existing_variations[{{ $variation->id }}][is_active]" value="1" {{ $variation->is_active ? 'checked' : '' }}>
                                <span class="checkmark"></span>
                                Active Variation
                            </label>
                        </div>
            ...
            <div class="new-variations">
                ...
                <div class="variations-container" id="variations-container">
                    <!-- New variations will be added here dynamically -->
                </div>
...
<template id="variation-template">
    <div class="variation-item new-variation">
        <div class="variation-header">
            <h4>New Variation <span class="variation-number">1</span></h4>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('click', function(e) {
        if (e.target.closest('.remove-existing-image')) {
            const imageBox = e.target.closest('.existing-image');
            const imageId = e.target.closest('.remove-existing-image').dataset.imageId;
    ...
    document.addEventListener('click', function(e) {
        if (e.target.closest('.remove-existing-variation')) {
            const variationItem = e.target.closest('.existing-variation');
            const variationId = e.target.closest('.remove-existing-variation').dataset.variationId;
            
            // Create hidden input to mark variation for deletion
            const deleteInput = document.createElement('input');
            deleteInput.type = 'hidden';
    ...
    function reindexVariations() {
        const variations = variationsContainer.querySelectorAll('.new-variation');
    ...
    // Initialize image previews for existing variation images
    document.querySelectorAll('.variation-image-box .image-input').forEach(input => {
        input.addEventListener('change', function() {
            setupImagePreview(this, this.closest('.variation-image-box'));
        });
    });
});
</script>
